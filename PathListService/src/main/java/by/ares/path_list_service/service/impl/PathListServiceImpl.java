...
public class PathListServiceImpl implements PathListService {
    ...
    public PathListDto findById(UUID id) {
        return pathListRepository.findById(id)
                .map(pathList -> {
                    PathListDto dto = pathListDtoMapper.map(pathList);
                    CompanyDto company = companyClient.findById(pathList.getCompanyId());
                    assignDto(dto, company, pathList.getCarId(), pathList.getCarDriverId());
                    return dto;
                })
    ...
    public PathListDto save(PathListCreationRequest request) {
        var carDto = carClient.findById(request.carId());
        var carDriverDto = carDriverClient.findById(request.carDriverId());
        var companyDto = companyClient.findById(request.companyId());
        var pathList = pathListRequestMapper.map(request);
        var savedRoute = routeService.save(request.routeCreationRequest());
        pathList.setRoute(savedRoute);
        var seria = seriaRepository.findById(request.seriaDto().getId())
                .orElseThrow(() -> new RuntimeException("Seria not found"));
        ...
        pathListDto.setRoute(routeDtoMapper.map(savedRoute));
        pathListDto.setSeria(seriaDtoMapper.map(seria));
        return pathListDto;
    }
